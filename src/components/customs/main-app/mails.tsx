...
function MailNotification() {
  ...
  const { data: notifications, isLoading } = useGetNotificationsQuery({ notificationPage, limit },
        ...
        <div className="flex flex-col px-4 ">
          ...
          <div className="flex flex-col gap-3 pb-[8rem]">
            {notifications?.notifications?.map((notification: NotificationTypes) => (
              <Link to={notification.url} key={notification._id} className="py-5 px-2 flex items-center gap-3 border-b border-[#3E3D3D] hover:bg-gray-900 duration-200 inter">
                ...
                <div>
                  ...
                  <h1 className="text-[9px] italic text-[#FFFFFF] font-normal line-clamp-1">{notification.description}</h1>
                  <h1 className="text-[#D25804] text-[8px] font-medium">Posted {moment(notification.createdAt).fromNow()}</h1>
                </div>
              </Link>
            ))}
          </div>
